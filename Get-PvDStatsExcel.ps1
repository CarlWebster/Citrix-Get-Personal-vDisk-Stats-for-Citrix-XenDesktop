...
ForEach($Catalog in $Catalogs)
{
	...
	If( $? -and $Null -ne $VMs )
	{
		...
		ForEach($VM in $VMs)
		{
			...
			If($VM.AssociatedUserFullNames.Count -gt 0)
			{
			...
			If ($PowerState -eq "On")
			{
				...
				Else
				{
					...
					ElseIf ($Null -ne $PvDPool -and $PvDOk -eq $false)
					{
		...
		Sort-Object VMname | `
		Export-CSV "$($pwd.path)\$($FileName)_PvD_Stats.csv" -NoTypeInformation -EA 0 4>$Null
...
If($SummaryObjects.Count -gt 0)
{
	...
	Sort-Object VMname | `
	Export-CSV "$($pwd.path)\Summary_PvD_Stats.csv" -NoTypeInformation -EA 0 4>$Null
...
$Excel = New-Object -com Excel.Application -EA 0 4>$Null
...
If($DoSummary)
{
	...
	If((Test-Path "$($pwd.path)\Summary_PvD_Stats.csv"))
	{
		...
		Remove-Item $csvFile -EA 0 4>$Null	
